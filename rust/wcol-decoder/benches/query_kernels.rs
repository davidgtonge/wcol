...
use wcol_decoder::bench::{
    build_group_key, mask_and, mask_count, mask_is_zero, mask_not, mask_or, mask_words, op_between,
    op_eq, op_gt, op_lt, rows_per_chunk, BenchColumn,
};
...
fn bench_group(c: &mut Criterion) {
    ...
    c.bench_function("read_value_f64_u32", |b| {
        b.iter(|| {
            let mut acc = 0f64;
            for row in 0..rows {
                acc += col_a.read_value_f64(row);
            }
            black_box(acc);
        })
    });
