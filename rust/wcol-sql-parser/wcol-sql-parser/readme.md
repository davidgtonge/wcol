Minimal SQL parser to parse these sorts of queries.

V1
 - no positional group by or order by

Canonical DuckDB ClickBench queries. Verify-parity runs these on DuckDB and rewrites for wcol where needed (e.g. COUNT(DISTINCT x) -> approx_count_distinct(x)).
